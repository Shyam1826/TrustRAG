...
1. PURPOSE & ROLE:
   ...
   - Role: Query pre-processing, conversational noise reduction, and entity routing.
   - Purpose: Strips conversational filler phrases and preambles from user queries and
     performs dynamic metadata entity routing against known document corpora and entity
     dictionaries to enable targeted document-level pre-filtering and focused topic search.

2. INPUT (IP):
   - query (str): Raw user query string (e.g., "What are the academic projects of Vaishalee?").
...
3. PROCESS UNDER THE HOOD:
   ...
   - Entity Routing: Parses query tokens against known document IDs, entity aliases (person names, emails),
     and document stems. Returns matched `(doc_id, matched_entity_token)`.
   - Strips matched entity tokens and possessives from intra-document search query when routed,
     focusing lexical/dense retrieval on pure section topics (e.g. "projects", "academic projects").
   - Collapses consecutive whitespace characters into a single space.

4. OUTPUT (OP):
   - str: Focused, standalone query string.
   - tuple[Optional[str], Optional[str]]: `(doc_filter, matched_entity)`.

5. LIBRARIES & DEPENDENCIES:
   - re: Standard library module for regex pattern matching and token extraction.
   - typing (Dict, List, Optional, Tuple): Type annotations.
================================================================================
"""
...
class QueryTransformer:
    """Transforms raw conversational queries and extracts document entity filters."""
    ...
    _STOP_TOKENS = {
        ...
        "this", "that", "all", "any", "some", "his", "her", "their", "candidate", "person"
    }
    ...
    def transform(
        ...
        """Strip conversational filler patterns, comparative noise, and entity names.

        Args:
            query: Raw user query string.
            entities_to_strip: Optional single entity name or list of entity names to remove.
        ...
        # If scoped to entities, strip the entity tokens and possessives
    ...
    def extract_doc_filter(
        ...
        """Detect if the query explicitly names one or more documents/entities in the corpus.
        ...
        # 1. Check direct entity map (e.g. 'vaishalee' -> 'Minimalist CV Resume', 'sanjeev' -> 'Sanjeev_M_Resume')
        if doc_entity_map:
        ...
        # 2. Check known doc_ids
        if known_doc_ids:
            for doc_id in known_doc_ids:
                doc_id_lower = doc_id.lower()

                # Exact substring match (e.g. 'karthik' in query)
                if doc_id_lower in query_lower and doc_id_lower not in self._STOP_TOKENS:
                    matched_docs[doc_id] = doc_id_lower

                # Token-level matching (e.g. 'sanjeev' in 'Sanjeev_M_Resume')
                doc_tokens = [
                    t.lower()
                    for t in re.split(r"[_ -]+", doc_id)
        ...
        return list(matched_docs.keys()), list(matched_docs.values())
